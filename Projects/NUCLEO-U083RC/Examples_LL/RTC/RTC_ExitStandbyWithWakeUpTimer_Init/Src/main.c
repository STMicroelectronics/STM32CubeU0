...
/**
  ...
  *          with the WUT through the STM32U0xx RTC LL API.
...
int main(void)
{
  ...
  if (LL_PWR_IsActiveFlag_SB() != 1)
  {
    ...
    LL_RTC_BKP_SetRegister(TAMP, LL_RTC_BKP_DR0, seconds);
  }
  else
  {
 ...
 /* Enable RTC APB clock  */ 
    ...
    uint32_t number_of_wakeup = LL_RTC_BKP_GetRegister(TAMP, LL_RTC_BKP_DR1) + 1;
    LL_RTC_BKP_SetRegister(TAMP, LL_RTC_BKP_DR1, number_of_wakeup);
    uint32_t start_time = LL_RTC_BKP_GetRegister(TAMP, LL_RTC_BKP_DR0);

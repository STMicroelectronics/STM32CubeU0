...
HAL_StatusTypeDef plat_init_nv_counter(void)
{
  ...
  BOOT_LOG_INF("Consistent BL2 NV Counter %d  = 0x%lx", PLAT_NV_COUNTER_3, (unsigned long)counter_value);
  ...
  BOOT_LOG_INF("Consistent BL2 NV Counter %d  = 0x%lx", PLAT_NV_COUNTER_4, (unsigned long)counter_value);
                                      ...
                                      uint32_t Data, uint32_t *Updated)
{
  ...
  *Updated = 1U;
  ...
  uwAddressNextWrite += NVCNT_ELEMENT_SIZE;
  uhNbWrittenElements++;
  BOOT_LOG_INF("Counter %d set to 0x%lx", CounterId, (unsigned long)Data);
  return HAL_OK;
}
...
static bool VerifyPageFullyErased(uint32_t Address, uint32_t PageSize)
{
  ...
  while ((counter < PageSize) && (readstatus == true))
  {

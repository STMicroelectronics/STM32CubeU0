...
#define FLASH0_SECTOR_SIZE 0x800
...
static int32_t Flash_ReadData(uint32_t addr, void *data, uint32_t cnt)
{
  ...
  else
  {
    ...
    BOOT_LOG_ERR("Double ECC error detected: FLASH_ECCR=0x%lx", (unsigned long int)FLASH->ECCR);
...
static int32_t Flash_EraseSector(uint32_t addr)
{
  ...
  for (i = 0; i < FLASH0_PAGE_SIZE / sizeof(uint32_t); i++)
  {
    if (pt[i] != 0xffffffff)
    {
#ifdef DEBUG_FLASH_ACCESS

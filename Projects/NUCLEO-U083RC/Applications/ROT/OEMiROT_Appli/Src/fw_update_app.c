...
static HAL_StatusTypeDef FW_UPDATE_APP_IMAGE(void)
{
  ...
  fw_image_dwl_area.DownloadAddr = IMAGE_SECONDARY_PARTITION_OFFSET;
...
static HAL_StatusTypeDef FW_UPDATE_DATA_IMAGE(void)
{
  ...
  /* Get Info about the download area */
  fw_image_dwl_area.DownloadAddr = DATA_IMAGE_SECONDARY_PARTITION_OFFSET;
...
static void FW_Valid_DataImage(void)
{
  ...
  const uint32_t ConfirmAddress = DATA_IMAGE_PRIMARY_PARTITION_OFFSET + DATA_PARTITION_SIZE - (sizeof(MagicTrailerValue) + sizeof(FlagPattern));
  if (LOADER_FLASH_DEV_NAME.ProgramData(ConfirmAddress, FlagPattern, sizeof(FlagPattern)) == ARM_DRIVER_OK)
  {
    printf("  -- Confirm Flag correctly written %lx %x \r\n\n", (unsigned long int)ConfirmAddress, FlagPattern[0]);
  }
  else
  {
...
static void FW_Valid_AppImage(void)
{
  const uint8_t FlagPattern[]={0x1, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
  0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
  const uint32_t ConfirmAddress = IMAGE_PRIMARY_PARTITION_OFFSET + IMAGE_PARTITION_SIZE - (sizeof(MagicTrailerValue) + sizeof(FlagPattern));

...
/**
  ******************************************************************************
  * @file    TouchSensing\TouchSensing_OptimizedAcq_LowPower\Src\main.c
...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define NO_ACTIVITY_TIME        500 /* Equivalent 5 seconds */
...
#define TKEY_DEB(NB) (MyTKeys[(NB)].p_Data->StateId == TSL_STATEID_DEB_DETECT)
...
__IO uint32_t uwCurrentActivityTimeCount = 0;
__IO uint32_t uwNoActivityTimeCount = 0;
__IO uint8_t ubIdle = 0;
__IO uint8_t ubActive = 0;
__IO uint8_t Debounce = 0;
...
int main(void)
{
  ...
  uint32_t uwCumulativeTimeCount;
  /* USER CODE END 1 */
  ...
  BSP_LCD_GLASS_ScrollSentence((uint8_t *)" TouchSensing Optimized Low Power ", 1, SCROLL_SPEED_MEDIUM);
  ...
  /* Set state machine to Active, corresponding to TS1, button without Comparator */
  ubIdle=0;
  ubActive=1;

  /* Disable the interconnect between Comparator and TSC */
  /* TSC measurement is done on TSC IO directly */
  HAL_SYSCFG_ClearTSCComparatorModeIO(SYSCFG_COMP_MODE_GRP6_IO1);
  ...
  /* End of calibration of TS1, disable LED */
  BSP_LED_Off(LED3);

  /* Enable the interconnect between Comparator and TSC */
  /* TSC measurement is done on Compartor output */
  ...
  /* Set state machine to Idle, corresponding to TS2, button with Comparator */
  ubIdle = 1;
  ubActive = 0;
  ...
  /* End of calibration of TS2, disable LED */
  BSP_LED_Off(LED4);
  ...
  /* Set state machine to Active, corresponding to TS2, button without Comparator */
  ubIdle= 0;
  ubActive = 1;
  ...
  /* Start RTC Timer for 10ms interval */
  HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, 20, RTC_WAKEUPCLOCK_RTCCLK_DIV16, 0x0);
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    if (tsl_user_Exec_IT() != TSL_USER_STATUS_BUSY)
    {
      ...
      {
        /* Disable the interconnect between Comparator and TSC */
        /* TSC measurement is done through TSC IO directly */
        HAL_SYSCFG_ClearTSCComparatorModeIO(SYSCFG_COMP_MODE_GRP6_IO1);
        HAL_SYSCFG_DisableTSCComparatorMode();
        ...
        if (!TKEY_CAL(0))
        {
          if (TKEY_DET(0))
          {
            ...
            /* Reset TSC application global variable */
            idx_bank_it = 0;

            /* Reset the no activity time counter, with last time count value */
            uwNoActivityTimeCount = uwCurrentActivityTimeCount;
          }
          else if (TKEY_REL(0) && (uwTSC_WakeupDetected == 1))
          {
            BSP_LED_Off(LED3);

            /* Check no activity time, if no activity time is over than 5s */
            /* Switch measurement in TS1 Real key with comparator, Idle */
            uwCumulativeTimeCount = uwCurrentActivityTimeCount;

            if ((uwCumulativeTimeCount - uwNoActivityTimeCount) >= NO_ACTIVITY_TIME)
            {
              /* Switch measurement on TS1 Real key */
              ubIdle = 1;
              ubActive = 0;
      ...
      if (ubIdle)
      {
        ...
        if (!TKEY_CAL(1))
        {
          ...
          else if (TKEY_REL(1) && (uwTSC_WakeupDetected == 1))
          {
            BSP_LED_Off(LED4);

            /* Switch measurement on TS2 Virtual key */
            ubIdle = 0;
            ubActive = 1;
...
static void MX_TSC_Init(void)
{
  ...
  */
  ...
  htsc.Init.SamplingIOs  = TSC_GROUP6_IO1| TSC_GROUP1_IO2;

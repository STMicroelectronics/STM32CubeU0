...
/* USER CODE BEGIN PV */
/* Private variables ---------------------------------------------------------*/
/* set to 1 after Joystick Sel push-button interrupt  */
...
int main(void)
{
  ...
  MX_TIM2_Init();
  /* USER CODE BEGIN 2 */
  /* Initialize the Joystick Sel push-button.
  ...
  while (1)
  {
    /* Test if user an action occurred on the Joystick Sel push-button */
    if (UserButtonStatus == 1)
    {
      HAL_Delay(200);
      UserButtonStatus = 0;
      /* Change the gain */
      if (hopamp1.Init.PgaGain == OPAMP_PGA_GAIN_2)
      {
